...
'cms':{//系统
	...
	'pre':function(){//代码高亮
		...
		if($("pre code").length){
			...
			$("<link>").attr({ rel: "stylesheet",type: "text/css",href: "//s3.pstatp.com/cdn/expire-1-M/highlight.js/9.12.0/styles/googlecode.min.css"}).appendTo("head");
			$.getScript("//s1.pstatp.com/cdn/expire-1-M/highlight.js/9.12.0/highlight.min.js", function(){
	...
	'dingwei':function() {// goToPhoto 播放页定位
            $("html,body").animate({scrollTop: $("#notice").offset().top}, 800);
        },
	'Sort': function() {//排序
		$(".sort-button").each(function(){
			$(this).on("click",function(e){
				e.preventDefault();
				$(this).parent().parent().parent().find(".tab-pane").each(function(){
	...
	'flickity':function(){//手机滑动
		if($(".ff-gallery").length){
			...
			$("<link>").attr({
				rel: "stylesheet",
				type: "text/css",
				href: "//s2.pstatp.com/cdn/expire-1-M/flickity/2.1.1/flickity.min.css"
			}).appendTo("head");
			$.getScript("//s1.pstatp.com/cdn/expire-1-M/flickity/2.1.1/flickity.pkgd.min.js", function(){
...
'image': {//图片
	'lazyload': function(){//延迟加载
		...
		$.getScript("//s2.pstatp.com/cdn/expire-1-M/jquery_lazyload/1.9.7/jquery.lazyload.min.js", function(response, status) {
			$("img.ff-img").lazyload({
	...
	'qrcode': function(){//生成二维码
		...
		$(".glyphicon-phone").on('show.bs.popover', function () {
			$(".glyphicon-phone").attr('data-content','<img class="ff-qrcode" src="//share.tvzb.cc/qr.php?size=200&text='+encodeURIComponent(feifei.browser.url)+'"/>');
		})
	},
	'vcode':function(){//安全码
...
'search': {//搜索
	...
	'autocomplete': function(){//3.5增加搜索结果控制 4.1update
		...
		if( $limit > 0){
			...
			$.getScript("//s1.pstatp.com/cdn/expire-1-M/jquery.devbridge-autocomplete/1.4.7/jquery.autocomplete.min.js", function(response, status) {
				$('.ff-wd').autocomplete({
...
'score': {//评分
	'raty': function(){
		if( $('.ff-score').length ){
			...
			$("<link>").attr({ rel: "stylesheet",type: "text/css",href: "//s1.pstatp.com/cdn/expire-1-M/raty/2.7.1/jquery.raty.css"}).appendTo("head");
			//
			$.getScript("//s0.pstatp.com/cdn/expire-1-M/raty/2.7.1/jquery.raty.min.js", function(response, status) {
...
'playurl': {//播放地址
	'tongji': function(){
		...
		//	$.getScript("/tongji/4.1/?"+~(-new Date()/36e5));
		}
	},
	'active':function(){
